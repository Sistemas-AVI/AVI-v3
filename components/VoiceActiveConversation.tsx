...
export const VoiceActiveConversation: React.FC<VoiceActiveConversationProps> = ({
  ...
  const circle3Opacity = useRef(new Animated.Value(0)).current;  // Animaciones según el estado del componente
  useEffect(() => {
  ...
  }, [isListening, isProcessing, isSpeaking]);  // Animación para el modo de escucha
  ...
  const speechToText = async (audioUri: string): Promise<string> => {
    try {
      ...
      const response = await fetch('https://api.openai.com/v1/audio/transcriptions', {
        ...
        // No pongas 'Content-Type' cuando usas FormData; fetch lo maneja automáticamente
      },
        body: formData,
      });
